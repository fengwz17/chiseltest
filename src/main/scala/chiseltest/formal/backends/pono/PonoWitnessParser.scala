...
object PonoWitnessParser {
  ...
  def read(lines: Iterable[String], parseMax: Int = 1): Seq[PonoWitness] = {
    var state: State = Start()
    val witnesses = mutable.ArrayBuffer[PonoWitness]()
    ...
    def parseLine(line: String): Unit = {
      ...
      def finishWitness(): State = {
        witnesses.append(
          PonoWitness(failed = failedBad, regInit = regInit.toMap, memInit = memInit.toMap, inputs = allInputs.toSeq)
        )
        regInit.clear()
        memInit.clear()
        inputs.clear()
